...
class getOnlyReestrAnalysTest extends TestCase
{
    ...
    {
        $object = new getOnlyReestrAnalys(
            (new workWithArrayRecordsErpn(array())),
            (new workWithArrayRecordsReestr(array()))
        );

        $reflection = new ReflectionClass(getOnlyReestrAnalys::class);
        ...
        $this->assertEquals($result[0]['inn'],"345014000000");
        $this->assertEquals($result[0]['type'],"ПНЕ");
        $this->assertEquals($result[0]["PDV_Reestr"],"23378.79");

        $this->assertEquals($result[1]['month'],"03");
        $this->assertEquals($result[1]['year'],"2017");
        $this->assertEquals($result[1]['inn'],"308978000000");
        $this->assertEquals($result[1]['type'],"ПНЕ");
        $this->assertEquals($result[1]["PDV_Reestr"],"118743.00");
        ...
        $this->assertEquals($object->getUniqKeyForBuildListDocuments(),
            array("03/2017/345014000000/ПНЕ",
                "03/2017/308978000000/ПНЕ"));
        return null;
    }
